...
int main(int argc, const char* argv[]) {
  ...
  const lf::mesh::Entity* entity_2 = mesh_p->EntityByIndex(0, 0);
  bool value = mesh_data_set(*entity_2);

  // Create MeshDataSet storing boolean values for each entity indicating if the
  // entity is on the boundary
  lf::mesh::utils::AllCodimMeshDataSet<bool> bd_flags{
      lf::mesh::utils::flagEntitiesOnBoundary(mesh_p)};

  // Define a lambda function that takes a point in the reference element of an
  // entity and returns a value
  ...
  // Generate a simple test mesh
  std::shared_ptr<lf::mesh::Mesh> mesh_p_2 =
      lf::mesh::test_utils::GenerateHybrid2DTestMesh(1);
  ...
  std::shared_ptr<lf::refinement::MeshHierarchy> mesh_seq_p{
      lf::refinement::GenerateMeshHierarchyByUniformRefinemnt(mesh_p_2, 3)};
  ...
  std::shared_ptr<lf::mesh::hybrid2d::MeshFactory> mesh_factory_2 =
      std::make_shared<lf::mesh::hybrid2d::MeshFactory>(2);

  // Add points (needs to be done before adding entities)
  mesh_factory_2->AddPoint(Eigen::Vector2d{0, 0});    // (0)
  mesh_factory_2->AddPoint(Eigen::Vector2d{1, 0});    // (1)
  mesh_factory_2->AddPoint(Eigen::Vector2d{1, 1});    // (2)
  mesh_factory_2->AddPoint(Eigen::Vector2d{0, 1});    // (3)
  mesh_factory_2->AddPoint(Eigen::Vector2d{0.5, 1});  // (4)

  // Add a triangle
  // First set the coordinates of its nodes:
  Eigen::MatrixXd nodesOfTria(2, 3);
  nodesOfTria << 1, 1, 0.5, 0, 1, 1;
  mesh_factory_2->AddEntity(
  ...
  // Build the mesh
  std::shared_ptr<lf::mesh::Mesh> mesh_2 = mesh_factory_2->Build();
  ...
  std::shared_ptr<lf::mesh::Mesh> mesh_p_3 = builder.Build();
